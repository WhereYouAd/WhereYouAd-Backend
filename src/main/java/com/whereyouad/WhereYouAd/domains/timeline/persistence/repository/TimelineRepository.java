...
public interface TimelineRepository extends JpaRepository<Timeline, Long> {
    ...
    @Query("SELECT t FROM Timeline t WHERE t.organization.id = :orgId")
    List<Timeline> findAllByOrganizationIdForUpdate(@Param("orgId") Long orgId);

    @Query("SELECT COALESCE(MAX(t.displayOrder), -1) FROM Timeline t WHERE t.organization.id = :orgId")
    int findMaxDisplayOrderByOrganizationId(@Param("orgId") Long orgId);
