...
public interface NotificationControllerDocs {
    ...
    @Operation(summary = "알림 채널 설정 변경", description = "브라우저 푸시, 이메일, 슬랙, 디스코드 알림 채널을 설정합니다. slackWebhookUrl/discordWebhookUrl은 ADMIN만 반영됩니다.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "변경 성공"),
            @ApiResponse(responseCode = "404", description = "해당 조직의 멤버가 아닙니다.")
    })
    ...
    @Operation(summary = "알림 목표 설정 변경", description = "비즈니스 리스크 알림 트리거(예산 소진율, 클릭 급증)를 설정합니다.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "변경 성공"),
            @ApiResponse(responseCode = "404", description = "해당 조직의 멤버가 아닙니다.")
    })
